...
- \( g \) = small network that turns the combined features into one score
...
If the correct one is already higher by margin \( m \) → loss is zero (good).  
If a wrong one scores higher → loss is large and the network is corrected.
